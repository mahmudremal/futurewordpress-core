...
class Option {
  ...
  private $file;
	...
	protected function setup_hooks() {
		...
		$this->plugin_slug = 'futurewordpress-project-scratch-domain';
		$this->plugin_name = 'futurewordpress-project-scratch-domain';
		$this->textdomain = 'futurewordpress-project-scratch-domain'; // str_replace('_', '-', $plugin_slug);
	...
	public function init() {
		$this->general = (object) wp_parse_args( apply_filters( 'futurewordpress/project/core/settings/general', [] ), [
			'page_title'					=> __( 'Configuration.', 'futurewordpress-project-scratch-domain' ),
			'menu_title'					=> __( 'Config', 'futurewordpress-project-scratch-domain' ),
			'role'								=> 'manage_options',
			'slug'								=> $this->plugin_slug,
			'page_header'					=> __( 'customization page.', 'futurewordpress-project-scratch-domain' ),
			'page_subheader'			=> __( 'Your setting panel from where you can control and customize.', 'futurewordpress-project-scratch-domain' ),
			'no_password'					=> __( 'A password is required.', 'futurewordpress-project-scratch-domain' ),
		] );
	...
	public function add_settings_link( $links ) {
		$settings_link = '<a href="options-general.php?page='.$this->general->slug.'">' . __( 'Settings', 'futurewordpress-project-scratch-domain' ) . '</a>';
		// array_push( $links, $settings_link );
		$links[] = $settings_link;
		$links[] = '<a href="https://www.fiverr.com/mahmud_remal/" target="_blank">' . __( 'Developer Support', 'futurewordpress-project-scratch-domain' ) . '</a>';
		return $links;
	}
	...
	private function settings_fields() {
		$settings = [];
		$settings = apply_filters( 'futurewordpress/project/core/settings/fields', $settings );
		return $settings;
	}
	...
	public function display_field( $args ) {
		...
		switch( $field['type'] ) {
			...
			case 'radio':
			case 'select_multi':
				$html .= apply_filters( 'futurewordpress/project/core/settings/fields/label', '<br/><span class="description">' . $field['description'] . '</span>', $field );
			break;

			default:
				$html .= apply_filters( 'futurewordpress/project/core/settings/fields/label', '<label for="' . esc_attr( $field['id'] ) . '"><span class="description">' . $field['description'] . '</span></label>' . "\n", $field );
			break;
		}
