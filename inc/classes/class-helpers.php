 ...
 */
 class Helpers {
	...
	protected function setup_hooks() {
		if( ! defined( 'FUTUREWORDPRESS_PROJECT_OPTIONS' ) ) {define( 'FUTUREWORDPRESS_PROJECT_OPTIONS', (array) get_option( 'futurewordpress-project-core-domain', [] ) );}
		add_filter( 'futurewordpress/project/core/system/getoption', [ $this, 'getOption' ], 10, 2 );
		add_filter( 'futurewordpress/project/core/system/isactive', [ $this, 'isActive' ], 10, 1 );

		add_filter( 'futurewordpress/project/core/database/countries', [ $this, 'databaseCountries' ], 10, 2 );
		add_filter( 'futurewordpress/project/core/database/countryflags', [ $this, 'countryFlags' ], 10, 1 );

		add_filter( 'futurewordpress/project/core/user/dashboardpermalink', [ $this, 'dashboardPermalink' ], 10, 2 );
		add_filter( 'futurewordpress/project/core/user/visitorip', [ $this, 'visitorIP' ], 10, 0 );


		add_filter( 'futurewordpress/project/core/filter/server/time', [ $this, 'serverTime' ], 10, 2 );
		add_filter( 'futurewordpress/project/core/filter/string/random', [ $this, 'generateRandomString' ], 10, 2 );

		add_filter( 'futurewordpress/project/core/filesystem/filemtime', [ $this, 'filemtime' ], 10, 2 );
		add_filter( 'futurewordpress/project/core/filesystem/uploaddir', [ $this, 'uploadDir' ], 10, 2 );
		add_filter( 'futurewordpress/project/core/mailsystem/sendmail', [ $this, 'sendMail' ], 10, 1 );


		add_filter( 'futurewordpress/project/core/notices/manager', [ $this, 'noticeManager' ], 10, 3 );

		
		add_action( 'wp_ajax_futurewordpress/project/core/filesystem/upload', [ $this, 'uploadFile' ], 10, 0 );
		add_action( 'wp_ajax_nopriv_futurewordpress/project/core/filesystem/upload', [ $this, 'uploadFile' ], 10, 0 );
		add_action( 'wp_ajax_futurewordpress/project/core/filesystem/remove', [ $this, 'removeFile' ], 10, 0 );
		add_action( 'wp_ajax_nopriv_futurewordpress/project/core/filesystem/remove', [ $this, 'removeFile' ], 10, 0 );
		add_action( 'admin_post_futurewordpress/project/core/filesystem/download', [ $this, 'downloadFile' ], 10, 0 );
		add_action( 'admin_post_nopriv_futurewordpress/project/core/filesystem/download', [ $this, 'downloadFile' ], 10, 0 );
	...
	public function uploadDir( $file = false, $force = false ) {
		...
		if( $this->theUploadDir === false ) {
			...
			$uploadDir[ 'basedir' ] = $uploadDir[ 'basedir' ] . '/' . apply_filters( 'futurewordpress/project/core/system/getoption', 'ftp-mediadir', 'futurewordpress' );
			$uploadDir[ 'baseurl' ] = $uploadDir[ 'baseurl' ] . '/' . apply_filters( 'futurewordpress/project/core/system/getoption', 'ftp-mediadir', 'futurewordpress' );
	...
	public function uploadFile() {
		check_ajax_referer( 'futurewordpress/project/core/verify/nonce', '_nonce' );
		...
		if( isset( $_FILES[ 'blobFile' ] ) || isset( $_FILES[ 'file' ] ) ) {
			...
			// 	$error = sprintf( __( 'File is larger then allowed range. (%d)', 'futurewordpress-project-core-domain' ), $file[ 'size' ][0] );
			// }
			...
			// 	$error = sprintf( __( 'File format (%s) is not allowed.', 'futurewordpress-project-core-domain' ), $extension );
			// }
			...
			if( $error === false && move_uploaded_file( $file[ 'tmp_name' ][0], $file[ 'full_path' ] ) ) {
				...
				// $meta['type'] = apply_filters( 'futurewordpress/project/core/validate/format', $meta['type'], $meta );
				$oldMeta[] = $meta;
				WC()->session->set( 'uploaded_files_to_archive', $oldMeta );
				wp_send_json_success( [
					'message'			=> __( 'Uploaded successfully', 'futurewordpress-project-core-domain' ),
					'dropZone'		=> $meta
				], 200 );
			} else {
				$error = ( $error ) ? $error : __( 'Something went wrong while tring to upload short clip video.', 'futurewordpress-project-core-domain' );
				wp_send_json_error( $error, 200 );
			}
		}
		wp_send_json_error( __( 'Error happens.', 'futurewordpress-project-core-domain' ) );
	}
	public function removeFile() {
		...
		if( isset( $fileInfo[ 'full_path' ] ) && file_exists( $fileInfo[ 'full_path' ] ) && ! is_dir( $fileInfo[ 'full_path' ] ) ) {
			...
			wp_send_json_success( __( 'File removed from server.', 'futurewordpress-project-core-domain' ), 200 );
		} else {
			wp_send_json_error( __( 'Failed to delete. Maybe File not found on server or your request doesn\'t contain file data enough.', 'futurewordpress-project-core-domain' ), 200 );
		}
	}
	public function downloadFile() {
		...
		if( isset( $fileInfo[ 'full_url' ] ) && isset( $fileInfo[ 'full_path' ] ) && file_exists( $fileInfo[ 'full_path' ] ) && ! is_dir( $fileInfo[ 'full_path' ] ) ) {
			...
			print_r( $fileInfo );
			wp_die( __( 'File not found', 'futurewordpress-project-core-domain' ), __( '404 not found', 'futurewordpress-project-core-domain' ) );
		}
	}
	...
	public function dashboardPermalink( $id, $user = 'me' ) {
		if( ! defined( 'FUTUREWORDPRESS_PROJECT_DASHBOARDPERMALINK' ) ) {
			$dashboard_permalink = apply_filters( 'futurewordpress/project/core/system/getoption', 'permalink-dashboard', 'dashboard' );
			$dashboard_permalink = site_url( $dashboard_permalink );
			define( 'FUTUREWORDPRESS_PROJECT_DASHBOARDPERMALINK', $dashboard_permalink );
		}
		$profile = ( apply_filters( 'futurewordpress/project/core/system/getoption', 'permalink-userby', 'id' ) == 'id' ) ? FUTUREWORDPRESS_PROJECT_DASHBOARDPERMALINK . '/' . ( ( $id ) ? $id : 'me' ) : FUTUREWORDPRESS_PROJECT_DASHBOARDPERMALINK . '/' . $user;
		return $profile . '/' . apply_filters( 'futurewordpress/project/core/profile/defaulttab', 'profile' );
	}
